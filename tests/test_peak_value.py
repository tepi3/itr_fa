...
def test_calculate_peak_value_lookback_flag(tmp_path, monkeypatch):
    ...
    # Verify top_candidates is present and contains entries
    assert "top_candidates" in res_a
    assert len(res_a["top_candidates"]) > 0
    assert res_a["top_candidates"][0]["date"] == "2024-05-15"
...
def test_calculate_peak_value_inr_dominance(tmp_path, monkeypatch):
    ...
    # Verify top_candidates reflects INR ordering
    assert "top_candidates" in res
    assert len(res["top_candidates"]) == 2
    assert res["top_candidates"][0]["value_inr"] == 8100
    assert res["top_candidates"][1]["value_inr"] == 8000

def test_calculate_peak_value_top_candidates_structure(tmp_path, monkeypatch):
    """
    Test that top_candidates contains correct structure and is sorted descending by INR value.
    """
    fake_cache_file = tmp_path / "sbi_cache.json"
    monkeypatch.setattr("core.sbi_rates.SBI_CACHE_FILE", fake_cache_file)
    ...
    mock_prices = [
        ...
        {"date": "2024-06-12", "close": 120.0},  # 120 * 10 * 84 = 100800 (Peak)
        {"date": "2024-06-13", "close": 105.0},  # 105 * 10 * 84 = 88200
        {"date": "2024-06-14", "close": 115.0},  # 115 * 10 * 84 = 96600
        {"date": "2024-06-17", "close": 95.0},   # 95  * 10 * 84 = 79800
    ]
    monkeypatch.setattr("core.calculator.get_historical_prices", lambda ticker, start, end: mock_prices)
    ...
    lot = {"id": "test_lot", "buy_date": "2024-01-01", "quantity": 10, "buy_price": 50.0, "sells": []}
    res = calculate_peak_value(lot, [], "AAPL", 2024, {}, mode='split')

    # Peak should be June 12 (highest INR)
    assert res["peak_date"] == "2024-06-12"
    assert res["value"] == 100800
    ...
    for i in range(len(tc) - 1):
        assert tc[i]["value_inr"] >= tc[i + 1]["value_inr"], \
            f"top_candidates not sorted: index {i} ({tc[i]['value_inr']}) < index {i+1} ({tc[i+1]['value_inr']})"

    # Verify the winner is first
    assert tc[0]["date"] == "2024-06-12"
    assert tc[0]["value_inr"] == 100800

    # Verify required fields in each candidate
    required_fields = {"date", "close_price", "qty", "ttbr", "rate_date", "source", "is_lookback", "value_inr"}
    for candidate in tc:
